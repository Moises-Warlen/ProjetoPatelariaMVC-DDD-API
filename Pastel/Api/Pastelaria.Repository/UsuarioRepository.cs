...
namespace Pastelaria.Repository
{
    ...
    public class UsuarioRepository : BaseRepository, IUsuarioRepository
    {
        ...
        public IEnumerable<UsuariosDto> Get()
        {
            ...
            using (var r = ExecuteReader())  // Executa o leitor para ler os resultados
            {
                while (r.Read())
                {
                    ...
                    usuario.Gestor = new UsuariosDto()
                    {
                        Nome = r.ReadAttr<string>("NomeGestor"),
                        IdUsuario = r.ReadAttr<int>("IdUsuarioGestor")
                    };
        ...
        // Obtém todos os gestores
        public IEnumerable<UsuariosDto> GetGestores()
        {
            ExecuteProcedure(Procedures.ListarGestores);  // Executa o procedimento armazenado para buscar gestores
            using (var r = ExecuteReader())  // Executa o leitor para ler os resultados
                return r.CastEnumerable<UsuariosDto>();  // Converte os resultados para um IEnumerable de UsuariosDto
        }
        ...
        public int Post(UsuariosDto usuario)
        {
            ...
            AddParameter("@IdGestor", usuario.Gestor.IdUsuario);
